...
#
# Time-stamp: <2023/10/01 13:55:03 (CST) daisuke>
#
...
for key, value in sorted (stars.items (), key=lambda x: x[1]['mag_V'], \
    ...
    array_appmag = numpy.append (array_appmag, stars[key]['mag_V'])
    array_absmag = numpy.append (array_absmag, stars[key]['absmag_V'])

# marker size and colour
marker_size   = 100.0 / (array_appmag + 2.0)**2
marker_colour = 4.0 - array_colour
...
# plotting data
ax.scatter (array_colour, array_absmag, \
            s=marker_size, c=marker_colour, cmap='Spectral')
