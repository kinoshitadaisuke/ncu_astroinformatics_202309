...
#
# Time-stamp: <2023/10/01 13:40:14 (CST) daisuke>
#
...
array_label  = numpy.array ([], dtype=str)
...
for key, value in sorted (stars.items (), key=lambda x: x[1]['mag_V']):
    ...
    array_label  = numpy.append (array_label, \
                                 f'{stars[key]["name"].decode ("utf-8")}')
...
# plotting data
for i in range (len (array_colour)):
    # marker size
    size = 15 - i * 0.5
    # plotting
    ax.plot (array_colour[i], array_absmag[i], \
...
# legend
ax.legend (bbox_to_anchor=(1.0, 1.1), loc='upper left')
